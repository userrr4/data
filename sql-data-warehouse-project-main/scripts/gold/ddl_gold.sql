...
Objectif du Script:
    ...
    La couche Gold représente les tables de dimensions et de faits finales (Schéma en Étoile)

    Chaque vue effectue des transformations et combine des données de la couche Silver
...
SELECT
    ROW_NUMBER() OVER (ORDER BY cst_id) AS customer_key, -- Clé de substitution
    ...
    CASE 
        WHEN ci.cst_gndr != 'n/a' THEN ci.cst_gndr -- CRM est la source primaire pour le genre
        ELSE COALESCE(ca.gen, 'n/a')  			   -- Recours aux données ERP
    END                                AS gender,
...
SELECT
    ROW_NUMBER() OVER (ORDER BY pn.prd_start_dt, pn.prd_key) AS product_key, -- Clé de substitution
